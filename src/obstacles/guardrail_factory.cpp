...
std::shared_ptr<GameObject> GuardrailFactory::get(Vector2d pos, float rotation, const std::shared_ptr<Scene> &scene) {
    auto obj = std::make_shared<GameObject>("guard-rail", "guard-rail", true);
    std::shared_ptr<Sprite> sprite = std::make_shared<Sprite>("./assets/obstacles/concrete.png", Color(0, 0, 0, 0),
                                                              false, false, 1, 20, 40.f);
