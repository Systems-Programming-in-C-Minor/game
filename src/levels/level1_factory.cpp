...
RaceLevel Level1Factory::get() {
    ...
    const auto high_score_ui = std::make_shared<GameObject>(
            "show-high-score", "high-score", Transform{Vector2d{10.f, -90.f}, Vector2d{}, 0.2f, 1.f});
    high_score_ui->add_component(
            std::make_shared<Text>(get_high_score("level 1", high_score_properties), "./assets/fonts/roboto/Roboto-Medium.ttf", 300, 10,
